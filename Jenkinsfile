node {
    withCredentials([[$class: 'UsernamePasswordMultiBinding',
        ...
        stage('Pull') {
            try {
                git branch: 'main', url: 'https://github.com/YoonjinOh0909/fisa05-jenkins-prac.git'
        ...
        stage('Deploy') {
            sshagent(credentials: ['ec2-fastapi-server']) {
                ...
                if (env.BUILD_NUMBER.toInteger() != 1) {
                    sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@54.180.56.17 "sudo docker rm -f docker-fastapi"')
                } else {
                    echo "Initial run (BUILD_NUMBER=${env.BUILD_NUMBER}). Skipping docker container removal."
                }
                sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@54.180.56.17 "pwd"')
                sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@54.180.56.17 "aws s3 cp ss3://ai-yoonjin-prd-bucket/iris_model.joblib iris_model.joblib --region ap-northeast-2"')
                sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@54.180.56.17 "sudo docker run --name docker-fastapi --env-file .env -e TZ=Asia/Seoul -p 80:8000 -v ./iris_model.joblib:/app/iris_model.joblib -d -t ${DOCKER_USER_ID}/fisa-app:${BUILD_NUMBER}"')
            }
